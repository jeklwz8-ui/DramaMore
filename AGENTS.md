# DramaMore 项目说明（给 Codex / 代码代理使用）

> 目标：让代码代理在修改 `DramaMore` 仓库时，能快速理解项目结构、现有链路、可安全修改点、事件需求与集成边界，避免误改播放页、广告链路和数据层。
>
> 本文档基于当前仓库源码整理，适合放在仓库根目录作为 `CODEX_PROJECT_GUIDE.md` 或 `AGENT_GUIDE.md`。
...
> 5. 只加最基础可运行实现，不做复杂通知 UI

### 12.4 给语言页加 Banner 广告容器逻辑
...
### 13.3 历史分页疑似存在 offset 逻辑问题
`HistoryActivity` 的分页偏移可能存在错误，需要谨慎验证后再改。

### 13.4 Follow 页数据并非纯本地
...
4. 验证步骤
5. 风险说明
6. 尚未完成项（如缺少 `google-services.json`、广告位未配置等）
