...
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“语言页仅显示语言名称”改动编译通过。
- 2026-04-09：本地化体验优化：新增内容语言本地持久化机制（`ContentLanguageHelper`），用户选择语言后会写入本地偏好；应用重启时若 SDK 未返回语言配置，会自动回填上次选择语言，不再要求重复选择。
- 2026-04-09：语言设置入口统一：`LanguageActivity` 与 `MineFragment` 的语言变更均改为调用 `ContentLanguageHelper.setSelectedContentLanguages(...)`，确保 SDK 状态与本地缓存一致。
- 2026-04-09：`App` 新增全局 `Application Context` 获取能力，供语言持久化读取/写入本地 `SharedPreferences` 使用。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“语言选择重启后保留”改动编译通过。
- 2026-04-09：按 `ShortPlay.voiceLanguage / originalLanguage` 实现播放页“原音/配音”两按钮功能：新增语音能力判断与缓存（同剧集记忆配音语种），点击“原音”可切回原音，点击上方“配音”可切到配音；若当前短剧不支持配音，上方“配音”按钮置灰并禁用。
- 2026-04-09：语音切换行为优化：切换原音/配音时保留当前播放进度（集数与秒数）重启当前播放页，避免丢失观看位置；并在 `onShortPlayPlayed(...)` 同步刷新语音能力状态。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“原音/配音两按钮功能实现”改动编译通过。
- 2026-04-09：沉浸式播放保护补充：进入沉浸式时强制关闭“配音”弹层并复位状态；同时在 `updateVoiceDubEntryExpanded(...)` 中增加沉浸式拦截，沉浸式下禁止展开“配音”字段。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“沉浸式不显示配音字段”改动编译通过。
- 2026-04-09：修复语音弹层按钮尺寸设置不生效问题：`showVoiceConfigPopup()` 中将 `popupWidth/popupHeight` 同步应用到 `contentView` 与 `PopupWindow`，并去除后续被锚点尺寸覆盖的逻辑；弹层位置改为按锚点水平居中显示。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“配音弹层尺寸生效修复”改动编译通过。
- 2026-04-09：播放页语音弹层交互继续完善：外层“配音”按钮尺寸改为与 `ll_voice_trigger`（原音按钮）一致；点击“原音按钮”可展开/收起，点击“配音按钮”或点击空白处都会恢复默认显示状态。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“配音按钮同尺寸 + 空白/按钮恢复默认”改动编译通过。
- 2026-04-09：播放页语音入口显示层级升级：点击“原音/箭头”后不再在底栏内部渲染“配音”，改为使用 Activity 最外层 `PopupWindow` 在锚点上方展示“配音”按钮，避免被底栏高度裁剪。
- 2026-04-09：生命周期防护补充：在 `onPause()` 与 `onDestroy()` 中主动关闭语音弹层并复位高亮态，防止页面切换后悬浮残留。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“语音按钮外层弹层显示”改动编译通过。
- 2026-04-09：修复播放页“配音”上层显示被挤压问题：底栏 `ll_play_bottom_actions` 与语音按钮容器 `tv_play_all` 均开启 `clipToPadding=false/clipChildren=false`；语音按钮移除上下内边距并底部锚定，确保点击后“配音”可在上方越界显示，不再被底栏高度裁剪。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“配音上层显示不被挤压”改动编译通过。
- 2026-04-09：修复播放页语音入口“点击无反应”问题：`ll_voice_trigger`、`tv_voice_original`、`iv_voice_arrow` 统一绑定同一点击事件，确保点击文本/箭头/触发区都能显示上方“配音”。
- 2026-04-09：语音入口可见性与样式微调：`tv_voice_dub` 默认改为 `INVISIBLE`（保留占位），并下调字号与间距（`12sp`、`1dp`），保证点击后上方文案可见且“原音”位置稳定。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“点击无反应修复 + 上方配音可见性优化”改动编译通过。
- 2026-04-09：播放页语音入口触发区域修复：将展开“配音”按钮的点击绑定统一到 `ll_voice_trigger` 整个组件（含原音文字与箭头区域），点击后仅展开上方“配音”文案，不改变“原音+箭头”组件位置。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“整块触发展开配音且位置不变”改动编译通过。
- 2026-04-09：播放页右下语音按钮样式优化：`tv_play_all` 高度收敛为 `32dp`，去掉多余上下内边距；“原音+箭头”改为底部锚定，“配音”显示在其上方显隐，从而避免背景过大并规范“原音”位置。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“语音按钮背景收敛与原音位置规范”改动编译通过。
- 2026-04-09：修复播放页语音入口位移问题：`tv_voice_dub` 收起态由 `GONE` 改为 `INVISIBLE`（保留上方占位），点击“原音/箭头”时“原音+箭头”位置保持不动，仅上方“配音”显隐切换。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“原音位置固定、仅配音显隐”改动编译通过。
- 2026-04-09：播放页语音入口颜色交互修复：`tv_voice_original` 与 `iv_voice_arrow` 默认显示白色，仅在点击展开态时高亮为红色；收起后恢复白色，未改动按钮位置与其他业务逻辑。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“点击时高亮、默认白色”改动编译通过。
- 2026-04-09：播放页语音入口交互继续修复：`iv_voice_arrow` 与 `tv_voice_original` 统一为同一触发展开/收起行为；并删除 `tv_play_all` 点击后“从头播放第1集”的旧逻辑，避免误触发重播，按钮位置与布局保持不变。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“箭头与文字同效 + 移除从头播放触发”改动编译通过。
- 2026-04-09：播放页语音入口交互再次调整：展开“配音”按钮的触发点由箭头改为 `tv_voice_original`（原音文案本身）；点击该文案时仅执行展开/收起，不再触发其旧点击链路。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“点击原音文案展开配音按钮”改动编译通过。
- 2026-04-09：播放页语音入口交互继续修复：`tv_voice_dub` 默认隐藏，改为仅在点击 `iv_voice_arrow` 时展开显示上方“配音”按钮；箭头会随展开状态旋转，不改动其他播放主链路逻辑。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“点击箭头才弹出上方配音按钮”改动编译通过。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat clean :app:assembleDebug`，用于清理旧编译产物并确认“删除配音/原音模式功能”后的主工程可完整重编译通过。
- 2026-04-09：补充清理语音模式残留：删除 `player_overlay_for_stream.xml` 中历史注释块（`tv_voice_mode/不限`）与 `DramaPlayActivity` 内空的 `updateVoiceModeLabel()` 占位方法，避免后续误判为功能仍在使用。
- 2026-04-09：功能验证日志：已重新执行 `./gradlew.bat :app:assembleDebug`，本次“配音/原音功能残留清理”改动编译通过。
- 2026-04-09：已按“删除配音/原音功能”要求完成源码清理：删除 `VoiceModeHelper`、播放页语音模式切换与菜单逻辑、`tv_voice_mode` 相关布局与按钮绑定；`tv_play_all` 已恢复为“播全集”入口（点击回到第 1 集）。
- 2026-04-09：`HomeFragment` 与 `RecommendFragment` 已移除语音模式筛选依赖，列表恢复为直接使用 `requestFeedList` 返回数据，不再保留 `MODE_DUBBED / MODE_ORIGINAL / MODE_ALL` 分支。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“删除配音/原音相关功能代码”改动编译通过。
- 2026-04-09：已彻底移除“配音/原音/不限”中的 `不限` 模式代码：`VoiceModeHelper` 仅保留 `MODE_DUBBED` 与 `MODE_ORIGINAL`，并删除 `MODE_ALL` 常量与对应文案返回；播放页右侧旧三选一入口已改为复用二态语音切换逻辑。
- 2026-04-09：推荐页语音过滤兜底逻辑已同步去除 `MODE_ALL` 判断，改为“过滤结果为空时回退未过滤列表”的通用兜底，不再依赖三态模式常量。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“移除不限三态模式相关代码”的改动编译通过。
- 2026-04-09：已删除播放页旧版“配音/原音/不限”三选一语音模式入口相关代码（`CustomOverlayView` 内 `showVoiceModeDialog`、单选弹窗创建与对应 dismiss 逻辑），右侧 `tv_voice_mode` 点击改为复用当前页面的二态语音切换入口，不再出现“原音/配音/不限”三项选择。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“删除三选一语音模式功能并切到二态入口”的改动编译通过。
- 2026-04-09：播放页语音入口交互再次修正：点击 `tv_play_all` 后不再弹出上拉菜单，改为仅显示页面内“配置态”按钮（与设计图一致）；首次点击只展开按钮样式，后续点击直接在原音/配音间切换。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“移除上拉菜单，仅保留配置按钮交互”的改动编译通过。
- 2026-04-09：播放页语音入口视觉补充：`tv_play_all` 已改为“默认收起态 + 点击展开态”，默认显示单行“原音 + 上箭头”（图1），点击后切为双行“配音 / 原音 + 红色上箭头”（图2）；上拉菜单交互已在后续版本移除。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“语音入口收起/展开态样式联动”改动编译通过。
- 2026-04-09：播放页底部 `tv_play_all` 已从“播全集”入口改为“原音/配音”切换入口，样式调整为设计图同款双行深色按钮（上方“配音”，下方当前模式 + 上箭头），并保留原播放主链路稳定。
- 2026-04-09：功能验证日志：点击该入口会弹出上拉语音模式菜单（原音/配音）；基于当前 `ShortPlay.voiceLanguage` 与 `originalLanguage` 判断可切换性，若当前剧集不支持配音则“配音”项置灰且不可点击；切换后底部入口与右侧 overlay 语音文案会同步刷新。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，本次“tv_play_all 改为原音/配音切换入口 + 上拉菜单 + 不可配音置灰”改动编译通过。
- 2026-04-09：已将一次完整代码扫描后的实际工程现状补充进 `AGENTS.md`，新增主工程识别、初始化链路真实状态、Firebase 未正式接入判断、播放页高频插入点、关注页混合数据来源、历史分页风险点等内容，便于后续任务直接命中真实代码结构，减少重复阅读成本。
- 2026-04-09：功能验证日志：已执行 `./gradlew.bat :app:assembleDebug`，用于验证“补充 AGENTS.md 实际代码扫描说明”的本次文档更新未影响主工程构建。
